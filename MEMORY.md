...
## Incident: Gateway Crossover with AutoResearch (2026-05-04)

**Summary:** Implementing Hermes AutoResearch (`research` toolset) on the Siqui agent caused the DaemonCraft gateway to cross-contaminate with CLI/TUI gateway behavior. The agent began identifying as Pamplinas, sending messages via Telegram instead of Minecraft chat, and spawning research sub-agents that interfered with the Minecraft session.

**Root causes:**

1. **`research` added to `platform_toolsets.daemoncraft`** in `~/.hermes/config.yaml`. This forced ALL DaemonCraft agents to load web/CLI research tools (`run_research`, `delegate_task`, `web_search`, `browser`) — tools designed for terminal/IDE contexts, not Minecraft bots. The crossover was literal: a Minecraft bot tried to spawn browser sessions and delegate coding tasks.

2. **`research` added to Siqui's individual profile** (`~/.hermes/profiles/siqui/config.yaml`). Even after fixing the global platform toolsets, the profile still loaded `research` into the agent's tool registry.

3. **Model mismatch**: Profile had `kimi-for-coding` + `api.kimi.com/coding/v1` while the global config had `kimi-k2.6`. The coding endpoint returned HTTP 403 ("only available for Coding Agents"), causing the gateway to enter interrupt-recursion loops (`Interrupt recursion depth 3 reached`).

4. **Missing cast files**: `agents/casts/siqui.yaml` and `agents/prompts/rolemaster/siqui.md` had been deleted from the working branch after commit `53922de`, but the systemd service (`daemoncraft-siqui.service`) continued to reference them. On restart, `daemoncraft.py` failed to load the cast (`Cast 'siqui' not found`), yet the gateway service (`siqui.service`) kept running with a stale, manually-corrupted profile.

5. **SOUL contamination**: The profile's `SOUL.md` had been modified (either by research sub-agents or manual edits) to include CLI/TUI instructions (`You run inside a Python subprocess. You can use terminal and file tools`) and retained Pamplinas-first-person content from `SOUL-rolemaster.md` without the Siqui identity override being effective.

**Evidence from logs:**
```
[loop] Toolsets: ['research', 'minecraft', 'messaging', 'vision']
  ┊ 📨 send      telegram: "mc_perceive(type="status")"  0.2s [error]
[loop] Response: I don't have a direct connection to Minecraft tools from this CLI context.
  I'm Pamplinas — the Holodeck Director — but right now I'm speaking to you through the terminal...
```

**Fix applied:**
- Removed `research` from `platform_toolsets.daemoncraft` in global `~/.hermes/config.yaml`
- Removed `research` from Siqui profile `toolsets` and `platform_toolsets.cli`
- Updated model to `kimi-k2.6` + `base_url: https://api.moonshot.ai/v1` in both global and profile configs
- Restored `agents/casts/siqui.yaml` with corrected model and `vision` toolset only
- Restored `agents/prompts/rolemaster/siqui.md` from commit `53922de`
- Rebuilt profile `SOUL.md` from canonical sources (`SOUL-base.md` + `SOUL-rolemaster.md` + `siqui.md`)
- Restarted `siqui.service`

**Prevention rules (hard constraints):**
- **NEVER** add `research`, `terminal`, `file`, `web`, `browser`, `code_execution`, or `delegation` toolsets to `platform_toolsets.daemoncraft` or any DaemonCraft agent profile. These are CLI/TUI tools. A Minecraft bot has no browser, no terminal, and no filesystem access in the Hermes CLI sense.
- **NEVER** add `research` to `extra_toolsets` in any DaemonCraft cast YAML.
- **Model changes require explicit user confirmation** (already a principle, but worth repeating here because the `kimi-for-coding` → `kimi-k2.6` change was originally user-requested but the profile config was never updated to match).
- **Cast YAML and prompt files must not be deleted without updating the corresponding systemd service** (`daemoncraft-siqui.service`, `siqui.service`). If a cast is retired, stop and disable both services first.
- **Profile `SOUL.md` is auto-generated by `daemoncraft.py`**. Do not hand-edit it. If it drifts, run `daemoncraft.py update <cast>` or delete the profile dir and let the launcher rebuild it from canonical sources.

**Architecture note:**
Hermes has TWO gateway concepts that must not be confused:
- **Hermes CLI/TUI Gateway** (`hermes gateway run` for Telegram/Discord/CLI) — owns cognition for chat platforms, loads research/browser/terminal tools.
- **DaemonCraft Bot Gateway** (`siqui.service` running `hermes gateway run` with `platforms.daemoncraft.enabled=true`) — owns cognition for Minecraft bots, loads ONLY `minecraft`, `messaging`, `vision`, and `clarify` tools.

Both use the same `hermes gateway run` binary, but they are SEPARATE systemd services with SEPARATE `HERMES_HOME` directories and SEPARATE platform configurations. The only bridge between them is the bot server WebSocket (`ws://localhost:3002/ws`), which the DaemonCraft adapter consumes. There is no reason for a Minecraft bot to ever spawn a research sub-agent.
